exports.disappear = (element,callback = () =>{}) => {
    ...
    if (element.classList.contains("veri-appear")) {
    ...
    function startDisappear() {
        element.classList.add("veri-disappear");
...
exports.appear = (element,callback = () => {}) => {
    ...
    // Make sure that the element does have appearing animation
    if (element.classList.contains("veri-disappear")) {
        callback();
    }else {
        element.classList.add("veri-appear");
        element.addEventListener("animationend",() => {
            element.classList.remove("veri-appear");
            callback();
        });
    }
};